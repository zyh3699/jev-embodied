...
<div class="app-shell">
 <header class="header">
  <div class="brand"><div class="brand-mark">${icon("scan-line")}</div><div><strong>jev-embodied</strong><span>DECISION LAB</span></div></div>
  <div class="header-divider"></div><div class="header-context">机器人决策评测工作台</div>
  <nav class="view-switch" aria-label="工作模式"><button class="active" id="workbench-view" type="button">实验</button><button id="comparison-open" type="button">对比</button><button id="extensions-open" type="button">配置</button></nav>
  <div class="header-right"><span class="engine-label"><span class="dot"></span>SIM ONLINE</span><button class="icon-button mobile-settings" id="settings-open" title="实验参数" aria-label="实验参数">${icon("sliders-horizontal")}</button><button class="icon-button" id="export" title="导出实验记录" aria-label="导出实验记录">${icon("download")}</button></div>
 </header>
 <div class="body-grid">
 <div class="scrim" id="scrim"></div>
 <aside class="sidebar" id="sidebar">
  <div class="setup-intro"><div><span class="eyebrow">SETUP / 01</span><h1>实验设置</h1></div><button class="icon-button close-settings" id="settings-close" aria-label="关闭参数">${icon("x")}</button><p>选择任务与决策方式，然后从中间工作区启动。</p></div>
  <section><div class="section-topline"><h2><span class="section-index">01</span>任务</h2></div>
  ...
  <div class="divider"></div>
  <section class="input-section" id="input-section"><div class="section-topline"><h2>实时输入</h2><span class="eyebrow">WORLD / m</span></div><p class="input-context" id="input-context">实时观测</p><table class="input-table"><thead><tr><th>位置</th><th>X</th><th>Y</th><th>Z</th></tr></thead><tbody id="input-positions"></tbody></table><div class="input-contacts" id="input-contacts">等待观测</div></section>
  <details class="advanced-settings" id="advanced-settings"><summary><span class="summary-title">高级实验设置</span><span>种子 · 预算 · 扰动 · 速度</span></summary><section>
