...
实验日期为 2026-09-24。所有媒体均由本仓库保存的轨迹或相机帧生成，不引用其他项目的视频，导出阶段模型调用数为 0。

## 一句话结论

Jev 在 Panda 和 Meta-World 三项基础任务中保持了与 GPT-6 Astra 相同的成功数，同时显著减少运行时间和估算费用。开抽屉/开门任务中 Jev 为 4/4，GPT 为 3/4，但只有四个配对样本且 GPT 的失败包含接口/格式异常，不能据此宣称普遍能力提升。LIBERO 两组均未成功。
...
Jev 九局开发集结果为 9/9、63.98 秒、估算 $0.004853；由于对应的 GPT 只运行一局，该开发集不进入主配对图。
...
- 适配器：提供接近把手、贴合把手、拉抽屉和转动门四类子目标，并锁存已经完成的阶段，避免模型在接近/贴合之间来回切换。
