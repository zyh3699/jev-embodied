...
jev-embodied 用 MuJoCo 模拟 Franka Panda。模型可以选择预设技能，也可以根据每轮观察选择 XYZ 短步和夹爪动作。观察来源包括仿真状态、RGB-D 检测坐标和直接相机图像。浏览器负责显示与控制，Python 服务负责物理、模型请求和实验记录。安装命令见 [README](../README.md#-零机器人基础快速上手)，多模型实验见 [对比指南](COMPARISON.md)。
...
通过 `jev-embodied serve` 启动时，Key 存入系统钥匙串，服务重启后会恢复已保存的连接。URL、模型 ID、配置名称等信息单独保存在仓库外：

| 系统 | 连接信息目录 |
| --- | --- |
| macOS | `~/Library/Application Support/jev-embodied` |
| Windows | `%LOCALAPPDATA%/jev-embodied` |
| Linux | `$XDG_CONFIG_HOME/jev-embodied`，未设置时使用 `~/.config/jev-embodied` |
...
若只想临时使用配置，可运行 `jev-embodied serve --memory-only`，或设置 `EMBODIED_JEV_PERSISTENCE=memory`。这会跳过系统存储，Key 随服务退出而清除。
...
```bash
EMBODIED_MINICPM=1 EMBODIED_DEVICE=mps jev-embodied benchmark \
...
[openroboto 的适配器](https://github.com/openroboto-ai/jev-robot-control/blob/7a4ed8b72c3c17d7aa790678ed9660df67c10dd3/incremental_policy.py) 还使用 OpenRouter 的实验性地址 `https://openrouter.ai/api/alpha/decisions`、模型 `typesafe/jev-1.13`。在 jev-embodied中可通过 **Jev / 结构化决策 API** 填写这组完整地址和有访问权限的 OpenRouter Key。普通聊天接口权限不代表能访问该路由，本项目尚未实测这项服务。
...
```bash
jev-embodied serve --port 8090 --log-file runs/server.jsonl
```
...
pytest -q
jev-embodied benchmark --output runs/benchmark.json
